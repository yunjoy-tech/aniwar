...
func NewEquipHandler(actor *UserActor) *EquipHandler {
	...
	h.actor.RegisterProtoHandler(int32(pb.Protocols_PC2LS_EquipLevelUpReq), h.EquipLevelUpReq) // 装备升级
	h.actor.RegisterProtoHandler(int32(pb.Protocols_PC2LS_EquipWearReq), h.EquipWearReq)       // 装备穿戴
	h.actor.RegisterProtoHandler(int32(pb.Protocols_PC2LS_EquipUnWearReq), h.EquipUnWearReq)   // 装备卸下
...
func (h *EquipHandler) Init() error {
	// 初始化
	h.actor.Data.EquipData = &pb.PEquipData{
		Createtime: time.Now().Unix(),
		Equips:     make(map[uint64]*pb.PCommonEquipInfo),
	}
...
func (h *EquipHandler) SetDBData(dbData proto.Message) error {
	if dbVal, ok := dbData.(*pb.PEquipData); ok {
...
func (h *EquipHandler) EquipLevelUpReq(ctx context.Context, in *base.ProtoMsg) (proto.Message, error, int32) {
	var req pb.C2LS_EquipLevelUpReq
	...
	if err != nil {
		return nil, err, int32(pb.ErrorCode_EquipNotExist)
	}

	// 是否满级
	if equip.Level >= getEquipLevelLimit(equip.ConfigId) {
		return nil, fmt.Errorf("equip is max level"), int32(pb.ErrorCode_EquipIsMaxLevel)
	}
	// 消耗check
	...
	if !GetConsumeMgr(h.actor).CheckEnough(common.CURRENCY_ITEM_ID_2001, currencyCost) {
		return nil, fmt.Errorf("currency not enough %d", currencyCost), int32(pb.ErrorCode_CurrencyNotEnough)
	}
...
func (h *EquipHandler) EquipWearReq(ctx context.Context, in *base.ProtoMsg) (proto.Message, error, int32) {
	...
	if cfg == nil {
		return nil, fmt.Errorf("cfg not found %d", equip.ConfigId), int32(pb.ErrorCode_NotFoundConfig)
	}
	beastarCfg := excel.GetBeastarMgr().GetById(int32(req.CardId))
	if beastarCfg == nil {
		return nil, fmt.Errorf("cfg not found %d", req.CardId), int32(pb.ErrorCode_NotFoundConfig)
	}
	...
	rsp := &pb.LS2C_EquipWearRes{
		CommonData: &pb.CliComData{Equip: equipInfos, Card: cards},
	}
...
// 穿戴或者交换逻辑
func (h *EquipHandler) handleEquipWear(card *pb.CardData, equip *pb.PCommonEquipInfo) ([]*pb.PCommonEquipInfo, []*pb.CardData, pb.ErrorCode) {
	equipInfos := make([]*pb.PCommonEquipInfo, 0)
	cardInfos := make([]*pb.CardData, 0)
	var (
		errx error
		err  pb.ErrorCode
	)
	...
	return equipInfos, cardInfos, pb.ErrorCode_Success
}

func (h *EquipHandler) handleEquipUnWear(card *pb.CardData, typ uint32) (*pb.PCommonEquipInfo, pb.ErrorCode) {
	// 修正已穿戴装备的数据
	equip, err := h.GetEquip(card.EquipId[typ])
	if err != nil {
		return nil, pb.ErrorCode_EquipNotExist
	}
...
func (h *EquipHandler) EquipUnWearReq(ctx context.Context, in *base.ProtoMsg) (proto.Message, error, int32) {
	...
	if card.EquipId[req.Typ] == 0 {
		return nil, err, int32(pb.ErrorCode_EquipIsNotWear)
	}
	...
	// 消息返回
	rsp := &pb.LS2C_EquipUnWearRes{
		CommonData: &pb.CliComData{Equip: []*pb.PCommonEquipInfo{equip}, Card: []*pb.PClientCardInfo{h.actor.CardHandler.ToClientData(card)}},
	}

	return rsp, nil, 0
}

func (h *EquipHandler) buildEquipList() []*pb.PCommonEquipInfo {
	equipData := make([]*pb.PCommonEquipInfo, 0)
	for _, info := range h.actor.GetEquipData().Equips {
		equipData = append(equipData, &pb.PCommonEquipInfo{
...
// AddEquip 增加指定装备
func (h *EquipHandler) AddEquip(adds []*pb.PCommonEquipInfo, commonData *clidto.Comdata) error {
	limit := make(map[int32]int32)
	ret := make([]*pb.PCommonEquipInfo, 0)
	...
	for _, equip := range adds {
		...
		if h.actor.GetEquipData().Equips == nil {
			h.actor.GetEquipData().Equips = make(map[uint64]*pb.PCommonEquipInfo)
		}
...
func (h *EquipHandler) CreateEquip(baseId, num int32) ([]*pb.PCommonEquipInfo, error) {
	if num <= 0 {
		return nil, fmt.Errorf("create equip param err %d", num)
	}

	adds := make([]*pb.PCommonEquipInfo, 0)
...
func (h *EquipHandler) GetEquip(equipId uint64) (*pb.PCommonEquipInfo, error) {
...
// 装备喂养check
func (h *EquipHandler) equipCostCheck(costIds []uint64) pb.ErrorCode {

	for _, id := range costIds {
		// 是否存在
		equip, err := h.GetEquip(id)
		if err != nil {
			return pb.ErrorCode_EquipNotExist
		}

		// 是否穿戴中
		if equip.CardId != 0 {
			return pb.ErrorCode_EquipIsWear
		}
	}
...
// 是否可以增加经验
func canAddEquipExp(equip *pb.PCommonEquipInfo) bool {
	// 满级了
...
// 增加装备exp
func (h *EquipHandler) addEquipExp(equip *pb.PCommonEquipInfo, addExp int32) {
...
//	@receiver h
//	@param cardId 给定卡牌id
//	@return []*pb.PCommonEquipInfo 装备数据列表
//	@return error
func (h *EquipHandler) GetEquipList(cardId int32) ([]*pb.PCommonEquipInfo, error) {
	ret := make([]*pb.PCommonEquipInfo, 0)
	card, err := h.actor.CardHandler.GetCard(uint32(cardId))
	if err != nil {
...
// 获取主属性血量加成(绝对值)
func (h *EquipHandler) GetEquipMainAttrHp(equip *pb.PCommonEquipInfo) int32 {
	// 不是生命属性
	if equip.MainAttr != 1 {
		return 0
	}
...
// 获取副属性血量加成(百分比+绝对值)
func (h *EquipHandler) GetEquipSubAttrHp(equip *pb.PCommonEquipInfo) map[int32]int32 {
	// 不是生命属性
