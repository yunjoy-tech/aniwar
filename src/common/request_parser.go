...
// ParserReq 解析请求数据接口
func ParserReq(in *base.ProtoMsg, req proto.Message) (int32, string, error, cmd.ErrorCode) {
	var (
		err     error
		msgId   int32
		uid     string
	...
	if err != nil {
		...
		return msgId, uid, err, cmd.ErrorCode_DeSerializeError
	}
	//logger.Debugf("解析请求参数: in:%v, req:%T{%v}", in, req, req)

	return msgId, uid, nil, cmd.ErrorCode_Success
}
