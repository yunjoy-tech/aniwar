...
func TestPretty(t *testing.T) {
	...
	for _, sample := range tests {
		got, err := Pretty(sample)
		assert.NoError(t, err)
		assert.Equal(t, want, got)
	}

	bts, err := EncodePretty(map[string]int{"a": 1})
	assert.NoError(t, err)
	assert.Equal(t, want, string(bts))
}

func TestEncode(t *testing.T) {
	bts, err := Encode(testUser)
	assert.NoError(t, err)
	...
	bts, err = EncodeUnescapeHTML(&testUser)
	assert.NoError(t, err)
...
func TestEncodeToWriter(t *testing.T) {
	...
	err := EncodeToWriter(testUser, buf)
	assert.NoError(t, err)
	assert.Equal(t, `{"name":"inhere","age":200}
`, buf.String())
}
...
func TestWriteReadFile(t *testing.T) {
	...
	err := WriteFile("testdata/test.json", &user)
	assert.NoError(t, err)

	err = ReadFile("testdata/test.json", &user)
	assert.NoError(t, err)
