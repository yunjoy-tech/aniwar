	...
	if b != nil && messageId != pb.Protocols_Protocols_None {
		if u.ctx != nil {
			if messageId == pb.Protocols_PS2C_ErrorCodeNtf {
				...
				if err != nil {
					logger.Warn("OnNetMessage, HandleDeprecatedMsg Marshal, reply error:", u.String(), messageId, messageId, fmt.Errorf(": %w", err).Error())
				}
				err = u.ReplyWithBody(int32(messageId), reqIdx, errCode, b) // errCode不走加密
				if err != nil {
					logger.Warn("OnNetMessage, HandleDeprecatedMsg, reply error:", u.String(), messageId, messageId, fmt.Errorf(": %w", err).Error())
				}
			} else {
