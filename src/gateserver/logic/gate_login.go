...
func (s *GateServer) HandleLoginGame(c *tcpx.Context, session *cmd.UserSession, messageID int32, data []byte, reqIdx uint32) ([]byte, *base.RpcError) {
	var req cmd.C2G_LoginGameReq
	err := base.UnmarshalData(data, &req)
	if err != nil {
		return nil, &base.RpcError{Err: err, Code: int32(cmd.ErrorCode_DeSerializeError)}
	}

	//if conf.GConf().Base.VersionCheck {
	//	err = s.VersionCheck(req.GetVersion())
	//	if err != nil {
	//		return nil, &base.RpcError{Err: err, Code: int32(cmd.ErrorCode_VersionLimit)}
	//	}
	//}
	...
	if session == nil {
		...
		if session == nil {
			return nil, &base.RpcError{Err: fmt.Errorf("account[%s] session not found, req=%+v, err=%v ", accountId, req, err),
				Code: int32(cmd.ErrorCode_ReLogin)}
		} else if err != nil {
	...
	return nil, &base.RpcError{Err: fmt.Errorf("ErrorCode,%v", cmd.ErrorCode_InternalError),
		Code: int32(cmd.ErrorCode_InternalError)}
}
