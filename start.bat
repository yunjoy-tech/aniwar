...
if %Debug% EQU  "l" (
   ...
   echo ">>>>>>>>>>>>>>lobby"
    ...
    --pprof-addr %LOBBY_PPROF_PORT% --log %LOG_CONFIG%
   )
...
ENDLOCAL
   ...
   start %ACTOR_APPID% %BIN_PATH%\dapr.exe run -c %DAPR_CONFIG% -a %ACTOR_APPID% ^
    ...
    --in-addr %ACTOR_APP_PORT% --grpc-port %ACTOR_GRPC_PORT% --pprof-addr %ACTOR_PPROF_PORT% --log %LOG_CONFIG%
   )
...
if %Debug% EQU  "lo" (
   ...
   echo ">>>>>>>>>>>>>>login"
    ...
    --grpc-port %LOGIN_GRPC_PORT% --pprof-addr %LOGIN_PPROF_PORT% --log %LOG_CONFIG%
   )
...
if %Debug% EQU  "g" (
   ...
   --grpc-port %GATE_GRPC_PORT% --pprof-addr %GATE_PPROF_PORT% --log %LOG_CONFIG%
   )
...
if %Debug% EQU  "b" (
   ...
   --web-addr 18001 --pprof-addr %BILL_PPROF_PORT% --log %LOG_CONFIG%
   )
...
if %Debug% EQU  "i" (
   ...
   --pprof-addr %IDIP_PPROF_PORT% --log %LOG_CONFIG%
   )
...
if %Debug% EQU  "gu" (
   ...
   --pprof-addr %GUIDE_PPROF_PORT% --log %LOG_CONFIG%
   )
